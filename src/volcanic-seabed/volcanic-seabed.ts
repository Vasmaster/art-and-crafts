...
import {TerrainField, buildTerrainField, heightAt, pickVents} from './terrain'
...
// Particle sprites, generated by tools/make_particle_textures.py. White with the
// shape in the alpha channel, so the swarm can tint them from the same ramp as
// everything else in the scene.
const SMOKE_TEXTURE = 'assets/volcanic-seabed/smoke.png'
const EMBER_TEXTURE = 'assets/volcanic-seabed/ember.png'
const BUBBLE_TEXTURE = 'assets/volcanic-seabed/bubble.png'
...
interface Built {
  ...
  ventY: number
  /**
   * Every vent on the plate, in plan. One until the sculpt has been sampled, then
   * spread along the hot ridge. The particle spawn closures read this list live, so
   * the columns move to the real vents the moment the model arrives.
   */
  vents: {x: number, z: number}[]
  /** Surface height under each vent, refreshed every frame as the plate swells. */
  ventYs: number[]
  pools: ecs.Eid[]
  /**
...
const buildVent = (
  ...
  // One pool per vent. They are created against the single fallback vent here and
  // re-made once the sculpt has been sampled and the real vents are known.
  out.pools = out.vents.map((v, i) => {
    const e = child(world, root, v.x, y + 0.012, v.z)
    ecs.SphereGeometry.set(world, e, {radius: SUMMIT_R * (i === 0 ? 0.78 : 0.52)})
    hotMaterial(world, e, 255, 120, 24)
    return e
  })
  out.pool = out.pools[0]
...
const buildParticles = (world, root: ecs.Eid, out: Built, rng: () => number, floorY: number) => {
  ...
  out.swarms.push(createSwarm(world, root, {
    // Split across three vents now, so the pool has to be bigger for each column to
    // still read as a column rather than a few stray puffs.
    count: 78,
    radius: 0.055,
    shape: 'sprite',
    ...
    spawn: (r, heat) => {
      const a = r() * Math.PI * 2
      const d = r() * 0.045
      const k = Math.min(out.vents.length - 1, Math.floor(r() * out.vents.length))
      const v = out.vents[k]
      return {
        p: [v.x + Math.cos(a) * d, out.ventYs[k] + 0.03, v.z + Math.sin(a) * d],
        v: [(r() - 0.5) * 0.06, 0.13 + 0.16 * heat + r() * 0.05, (r() - 0.5) * 0.06],
        span: mix(2, 3.4, r()),
      }
    },
    growth: age => 0.5 + 2.4 * age,
    // Warm near the vent, cooling to a pale drifting haze. Kept well clear of black:
    // a dark particle over dark water reads as a hole, which is exactly what the old
    // tetrahedra did.
    colour: (age, heat) => [
      Math.round(mix(mix(196, 236, heat), 150, age)),
      Math.round(mix(mix(170, 206, heat), 150, age)),
      Math.round(mix(mix(150, 178, heat), 156, age)),
      fade(age) * (0.52 + 0.38 * heat),
    ],
  }, rng))
  ...
  out.swarms.push(createSwarm(world, root, {
    ...
    spawn: (r, heat) => {
      ...
      const v = out.vents[k]
      return {
        p: [v.x + Math.cos(a) * 0.02, out.ventYs[k] + 0.02, v.z + Math.sin(a) * 0.02],
        v: [
...
/**
 * Spread the vents along the hot ridge, and give each one a pool.
 *
 * Runs once, the first frame after the sculpt has been sampled. The particle spawn
 * closures read `b.vents` live rather than capturing it, so the smoke columns move to
 * the real vents on the same frame without anything being rebuilt.
 */
const placeVents = (world, b: Built, count: number) => {
  if (!b.field || count < 1) {
    return
  }
  // Seeded with the hand-tuned vent so the main one stays where it was put.
  const vents = pickVents(b.field, count, {x: b.ventX, z: b.ventZ})
  if (!vents.length) {
    return
  }
  b.vents = vents
  b.ventYs = vents.map(v => heightAt(b.field, v.x, v.z, 0))

  // The pools were built against the fallback vent, so rebuild the set to match.
  b.pools.forEach(e => world.deleteEntity(e))
  b.pools = vents.map((v, i) => {
    const e = world.createEntity()
    world.setParent(e, b.rootEid)
    world.setPosition(e, v.x, b.ventYs[i] + 0.012, v.z)
    // The first vent is the main one; the others are smaller, so the plate has a
    // focus instead of three identical chimneys.
    ecs.SphereGeometry.set(world, e, {radius: SUMMIT_R * (i === 0 ? 0.78 : 0.5)})
    hotMaterial(world, e, 255, 120, 24)
    return e
  })
  b.pool = b.pools[0]
...
ecs.registerComponent({
  ...
  schemaDefaults: {
    ...
    // and not an absolute.
    ventLift: 0.012,
    // How many vents to spread along the hot ridge once the sculpt has been sampled.
    // One reads as a single chimney in the middle of a plate that is hot right across
    // its width; three follows the band the vertex group actually describes.
    ventCount: 3,
  ...
  add: (world, component) => {
    ...
    const out: Built = {
      ...
      vents: [{x: useModel ? schema.ventX : 0, z: useModel ? schema.ventZ : 0}],
      ventYs: [useModel ? schema.ventHeight : SUMMIT_Y],
      pools: [],
      field: null,
      formations: null,
      seated: false,
  ...
  tick: (world, component) => {
    ...
    b.pools.forEach((pool, i) => {
      // Each vent breathes a little out of step with the others, so the plate reads
      // as several independent things venting rather than one thing copied.
      const off = i * 1.7
      const beat = 0.5 + 0.5 * Math.sin(data.elapsed * 2.6 + off)
      ecs.UnlitMaterial.mutate(world, pool, (m) => {
        m.r = Math.round(c.r)
        m.g = Math.round(c.g * (0.9 + 0.1 * beat))
        m.b = Math.round(c.b)
      })
    })

    // The pools breathe with the flicker instead of the jet cone that used to do it.
    b.pools.forEach((pool, i) => {
      const f = 0.85 + 0.4 * heat * (0.85 + 0.15 * Math.sin(data.elapsed * 11.3 + i * 2.1))
      world.setScale(pool, f, f * 0.75, f)
    })
    ...
    if (b.field) {
      if (!b.seated) {
        b.seated = true
        placeVents(world, b, schema.ventCount)
      ...
      const inf = influenceOf(data, schema)
      b.vents.forEach((v, i) => {
        b.ventYs[i] = heightAt(b.field, v.x, v.z, inf) + schema.ventLift
        if (b.pools[i]) {
          world.setPosition(b.pools[i], v.x, b.ventYs[i] + 0.012, v.z)
        }
      })
      b.ventY = b.ventYs[0]
    }
