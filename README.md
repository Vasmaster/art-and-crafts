# A.R.T. & Crafts — WebAR prototypes

Built on 8th Wall Studio's Image Targets template. The project's own work lives in the
**Volcanic Seabed** space (the entry space): the first of the three phygital blocks from
the project plan, where a hand-made resin block is the anchor for a WebAR layer that shows
what the resin cannot. See [`src/volcanic-seabed/README.md`](./src/volcanic-seabed/README.md).

`tools/` holds the scripts that generate the printable image target, build the target files
from a photograph, and author the scene. The sample spaces below are left intact as
reference.
...
The camera only starts in a secure context, so `http://<lan-ip>:8080` on a phone will
load the page but never get past the landing card. Use the 8th Wall Studio desktop app's
device connection, or a tunnel (`cloudflared tunnel --url http://localhost:8080`).

On desktop the scene is hidden until an image target is found, which never happens
without a camera — untick `hideUntilFound` on the **Seabed Root** component to look at
it, and put it back before testing tracking.

## Licence

Code is MIT ([LICENSE](./LICENSE)); creative work is CC BY-SA 4.0
([LICENSE-CONTENT](./LICENSE-CONTENT)). The 8th Wall sample assets inherited with the
template stay under their own terms.

---

## Studio: Image Targets (upstream template)
